# archivo: vistas/DetalleCompraView.py
...
from controller.controller_detalle_compra import Controlador_detalle_compra

class DetalleCompraView(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Gestión de Detalles de Compra")
        self.geometry("900x500")
        self.controlador = Controlador_detalle_compra()
    ...
    def _construir_interfaz(self):
        ...
        self.e_id_compra = tk.Entry(frame)
        self.e_id_producto = tk.Entry(frame)
        self.e_cantidad = tk.Entry(frame)
        self.e_precio_unitario = tk.Entry(frame)

        etiquetas = ["ID Detalle", "ID Compra", "ID Producto", "Cantidad", "Precio Unitario"]
        entradas = [self.e_id_detalle, self.e_id_compra, self.e_id_producto, self.e_cantidad, self.e_precio_unitario]

        for i, label in enumerate(etiquetas):
            tk.Label(frame, text=label).grid(row=i, column=0, sticky="e")
            entradas[i].grid(row=i, column=1)

        tk.Button(self, text="Agregar Detalle", command=self._agregar_detalle).pack(pady=10)
    ...
    def _cargar_detalles(self):
        ...
        try:
            ...
            if detalles:
                for d in detalles:
                    self.lista.insert(tk.END, f"ID: {d[0]} | Compra: {d[1]} | Producto: {d[2]} | Cantidad: {d[3]} | Precio: {d[4]}")
        except Exception as e:
            messagebox.showerror("Error", f"Error al cargar los detalles: {e}")

    def _agregar_detalle(self):
        id_det = self.e_id_detalle.get()
        id_compra = self.e_id_compra.get()
        id_producto = self.e_id_producto.get()
        cantidad = self.e_cantidad.get()
        precio = self.e_precio_unitario.get()

        if id_det and id_compra and id_producto and cantidad and precio:
            resultado = self.controlador.insertar_detalle(id_det, id_compra, id_producto, cantidad, precio)
            if resultado == 1:
...
if __name__ == "__main__":
    ...
    ventana = DetalleCompraView(root)
    ventana.mainloop()